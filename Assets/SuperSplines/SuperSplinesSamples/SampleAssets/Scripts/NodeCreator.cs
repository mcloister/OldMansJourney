...
public class NodeCreator : MonoBehaviour 
{
	...
	void Move( )
	{
		if( Input.GetKey( KeyCode.W ) || (Input.GetMouseButton(0) && Input.mousePosition.y/Screen.height > 0.7) )
			transform.position = transform.position + Vector3.forward * Time.deltaTime * 4f;
		if( Input.GetKey( KeyCode.S ) || (Input.GetMouseButton(0) && Input.mousePosition.y/Screen.height < 0.3) )
			transform.position = transform.position + Vector3.back * Time.deltaTime * 4f;
		
		if( Input.GetKey( KeyCode.A ) || (Input.GetMouseButton(0) && Input.mousePosition.x/Screen.width < 0.3))
			transform.position = transform.position + Vector3.left * Time.deltaTime * 4f;
		if( Input.GetKey( KeyCode.D ) || (Input.GetMouseButton(0) && Input.mousePosition.x/Screen.width > 0.7))
